...
- `qwen2.5:7b` for most users and Korean lecture reconstruction.
- `qwen2.5:3b` for lightweight testing.
- `llama3.2:3b` for general testing.

Ollama runs locally for free after installation, but speed and output quality depend on
the selected model and the user's CPU, RAM, GPU, and storage speed. PDF2Obsidian saves
the first Local AI result after light cleanup instead of running slow repeated retries
by default.

Large experimental models such as `qwen3.6` can require much more memory and may be
slow or unstable on ordinary PCs. They are not the default recommendation.
