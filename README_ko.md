...
> 현재 PDF 변환은 원본 PDF의 시각적 배치를 완벽히 복제하지 않습니다.
> 텍스트, 표, 링크, 일부 이미지를 추출한 뒤 Obsidian에서 재사용하기 쉬운
> Markdown으로 재구성합니다. 원본 레이아웃에 더 가까운 복원은 장기 개선 목표입니다.
...
같은 이름의 출력 폴더가 이미 있으면 기존 결과를 덮어쓰지 않고 `sample_2`처럼
번호가 붙은 새 폴더를 생성합니다.
...
`lecture.vtt` 같은 강의 자막 파일은 강의 개요, 핵심 개념, 시간대별 정리를 포함한 구조화된 학습 노트로 변환됩니다.
