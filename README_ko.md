...
- PDF 페이지 전체를 레이아웃 보존용 WebP 미리보기로 저장
...
- PDF 변환: 페이지 단위 WebP 미리보기, 레이아웃 인식 텍스트 추출, 내장 이미지 추출, Obsidian Markdown 출력
...
- PDF 페이지 전체를 WebP 미리보기로 저장
- 감지 가능한 PDF 표를 Markdown 표로 변환
- PDF 내부 이미지를 압축 WebP assets로 저장
- PDF 가져오기 모드 선택: Text Markdown, Page Image Markdown, Text + Page Image Markdown
...
- `Text Markdown`: 검색 가능한 텍스트와 PDF 내부 이미지를 추출합니다.
- `Page Image Markdown`: PDF 각 페이지를 `page_001.webp`, `page_002.webp`로 렌더링하고 페이지 이미지만 삽입합니다.
- `Text + Page Image Markdown`: 페이지 이미지를 먼저 넣고, 그 아래에 추출 텍스트와 내부 이미지를 넣습니다. 기본값입니다.

PyMuPDF가 표 구조를 감지할 수 있는 경우 Markdown 표로 출력합니다. 선이 없거나 복잡한 표는 수동 정리가 필요할 수 있습니다.
